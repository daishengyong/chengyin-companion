...
SMOKE_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/chengyin-gesture-discovery.XXXXXX")"
SMOKE_BIN="$SMOKE_ROOT/gesture-discovery-coordinator-smoke"
BUILD_ROOT="$(chengyin_swift_build_root "$PROJECT_DIR" gesture-discovery-smoke)"
...
xcrun swiftc \
  ...
  "$PROJECT_DIR/Sources/CompanionApp/CompanionGestureDiscoveryCoordinator.swift" \
  "$PROJECT_DIR/scripts/gesture-discovery-coordinator-smoke.swift" \
  "${CONTRACT_OBJECTS[@]}" \
  -o "$SMOKE_BIN"
