...
BUILD_ROOT="$(chengyin_swift_build_root "$PROJECT_DIR" workday-runtime-smoke)"
SMOKE_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/chengyin-workday-runtime.XXXXXX")"
SMOKE_BIN="$SMOKE_ROOT/workday-runtime-coordinator-smoke"
...
xcrun swiftc \
  ...
  "$PROJECT_DIR/Sources/CompanionApp/CompanionWorkdayPresentation.swift" \
  "$PROJECT_DIR/Sources/CompanionApp/CompanionWorkdayApplicationProjection.swift" \
  ...
  "$PROJECT_DIR/scripts/workday-runtime-coordinator-smoke.swift" \
  "${CONTRACT_OBJECTS[@]}" \
  -o "$SMOKE_BIN"
